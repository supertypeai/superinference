...
const ingoingContributionInference = async (githubHandle, token) => {
  let responseRepo,
    responseContrib,
    linksRepo,
    linksContrib,
  ...
  if (remainingRateContrib === 0 && linksContrib?.next) {
    messageContrib = `Hey there! Looks like the ingoing contributions above are from the latest ${dataContrib.length} repositories since you've reached the API rate limit 😉`;
  }
  ...
  let ingoingContribution= dataContrib.reduce((result, d) => {
  ...
  ingoingContribution = Object.fromEntries(
    Object.entries(ingoingContribution).sort(([, a], [, b]) => b - a)
  );

  return { ingoingContribution, messageContrib };
};

export default ingoingContributionInference;